...
################################################################################
# Load and clean Medical Expenditure Panel Survey (MEPS) 2014 data.
################################################################################
...
meps = meps %>%
  select(c(DUPERSID, RXDRGNAM:RXDAYSUP, RXSF14X:RXXP14X))
...
# which drug the strength is for), fix miscoded
meps = meps %>%
...
meps_pop = meps_pop %>%
  ...
  select(c("Person ID","AGE14X", "SEX", "PMUNAB42", "TTLP14X", "FAMINC14",
           "POVCAT14", "INSURC14","PMEDIN31","PMEDIN42","PMEDIN53","RACETHX",
           "RACEV2X"))
...
average_age_by_group <- meps_pop %>%
  ...
  summarize(mean_age = mean(as.numeric(AGE14X), na.rm = TRUE), .groups = 'drop')

meps_pop <- meps_pop %>%
  left_join(average_age_by_group, by = c("SEX", "RACEV2X", "RACETHX")) %>%
  mutate(AGE14X = ifelse(is.na(AGE14X), mean_age, AGE14X)) %>%
  select(-mean_age)
...
meps_pop = meps_pop %>%
  ...
  relocate(PMUNAB42, .after=POVCAT14)
