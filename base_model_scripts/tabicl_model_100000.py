...
start_total = time.perf_counter() # Track the whole script
...
def impute_age(df): # AKHILA: add the impute_age, impute_strength, and impute_day_supply functions to your code
    ...
    grp_meds = df.groupby(['Year', 'income_bracket', 'Insurance_coverage'], observed=False)['Age'].transform('median')
...
# Updated path for your Windows machine
cache_dir = r"C:\Users\Owner\vanessa\tabicl_cache"
# test if directory exists
os.makedirs(cache_dir, exist_ok=True)

# Setup cross validation
sgkf = StratifiedGroupKFold(n_splits=5, shuffle=True, random_state=42) # AKHILA: use these parameters for your sgkf and remove your other shuffling
fold_results          = []
...
for fold, (train_idx, val_idx) in enumerate(sgkf.split(data, y=data[target_variable], groups=data['Person_ID'])):
    ...
    # Impute Age within each fold, AKHILA: add the imputations for age, strength, and day supply for each fold here
    ...
    # Impute Day_Supply within each fold
    ...
    # --- TARGET DISTRIBUTION CHECK --- # AKHILA: I don't think you have to do this, but I ran this as a best practice to see how well drugs are distributed in folds and if any drugs are in testing but not training
    ...
    clf = TabICLClassifier(
        ...
        device="cuda",              # Use the RTX 4090
        use_amp="auto",             # Enables Mixed Precision (saves 50% VRAM)
        n_jobs=4,                   # Use 4 threads for data prep (12700K is fast)
        
        # --- Memory Killer Prevention ---
        support_many_classes=True,  # CRITICAL: Optimized logic for your 218 classes
        offload_mode="disk",        # FORCED: Moves memory-heavy patterns to your 8TB disk
        disk_offload_dir=cache_dir, # Point to your high-capacity storage
        batch_size=4,               # Start very low (4 or 8) to avoid memory spikes
        
        # --- Performance & Accuracy ---
        n_estimators=8,             # Good balance; increase to 16 only if stable
        kv_cache=False,             # Keep False (unsupported for >10 classes)
        softmax_temperature=0.9,    # Default (keeps predictions sharp)
        average_logits=True,         # Usually more stable for many classes
        use_fa3="auto"
    )
    ...
    gc.collect()
    if torch.cuda.is_available():
        torch.cuda.empty_cache() # Actually releases the 4090's VRAM for the next fold
...
# shuffle to separate refills, AKHILA: now we have to add this shuffling because it's not in sgkf anymore
...
data_2022 = impute_strength(impute_day_supply(impute_age(data_2022))) 
...
proba_2022 = final_clf.predict_proba(X_2022)
...
# Reorder columns to match reference encoder order — ensures ensemble alignment
proba_df = proba_df[['Observation_ID'] + list(le.classes_)]
proba_df.to_csv('tabicl_super_proba_2022.csv', index=False)
print(f'Probability output saved to tabicl_super_proba_2022.csv')
print(f'Shape: {proba_df.shape}')
...
print(f"Inference Time: {timedelta(seconds=end_pred - start_pred)}")
...
print("="*30)
print(f"Total Script Runtime: {timedelta(seconds=total_duration)}")
print(f"Final Fit Duration:   {timedelta(seconds=train_duration)}")
print("="*30)