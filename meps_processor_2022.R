...
meps_pop = meps_pop %>%
  ...
  select(c("Person ID","AGE22X", "SEX", "AFRDPM42", "TTLP22X", "FAMINC22",
           "POVCAT22", "INSURC22","PMEDIN31","PMEDIN42","PMEDIN53","RACETHX",
           "RACEV2X"))
...
average_age_by_group <- meps_pop %>%
  ...
  summarize(mean_age = mean(as.numeric(AGE22X), na.rm = TRUE), .groups = 'drop')

meps_pop <- meps_pop %>%
  left_join(average_age_by_group, by = c("SEX", "RACEV2X", "RACETHX")) %>%
  mutate(AGE22X = ifelse(is.na(AGE22X), mean_age, AGE22X)) %>%
  select(-mean_age)
...
meps_pop = meps_pop %>%
  ...
  relocate(AFRDPM42, .after=POVCAT22)
...
# export data as csv
write.csv(meps_clean,"data/meps_clean_2022.csv")