...
meps = meps %>%
  select(c(DUPERSID, RXDRGNAM:RXDAYSUP, RXSF20X:RXXP20X))
...
# fix dataset specific miscoding (remove trailing periods from Strength)
meps$Strength <- gsub("\\.+$", "", meps$Strength)
...
for (i in 1:length(drugs)){
  ...
  # Convert G/ML to MCG (mass and volume often directly given (e.g. 3 mg/20 mL);
  # if volume not given, assuming 1 mL)
  drug_data = drug_data %>%
...
meps_pop = meps_pop %>%
  ...
  select(c("Person ID","AGE20X", "SEX", "AFRDPM42", "TTLP20X", "FAMINC20",
           "POVCAT20", "INSURC20","PMEDIN31","PMEDIN42","PMEDIN53","RACETHX",
           "RACEV2X"))
...
average_age_by_group <- meps_pop %>%
  ...
  summarize(mean_age = mean(as.numeric(AGE20X), na.rm = TRUE), .groups = 'drop')

meps_pop <- meps_pop %>%
  left_join(average_age_by_group, by = c("SEX", "RACEV2X", "RACETHX")) %>%
  mutate(AGE20X = ifelse(is.na(AGE20X), mean_age, AGE20X)) %>%
  select(-mean_age)
...
meps_pop = meps_pop %>%
  ...
  relocate(AFRDPM42, .after=POVCAT20)
