...
#  Run this command to execute the full dataset with all features 
# and no class filtering:
#  python newNNapproach.py \

# Run this command to execute the full dataset with all features
# but only for the subset of classes with recall >= 0.5 in the CV results:
# python newNNapproach.py --feature-set expanded --class-filter-csv /Users/bastias/CUMed/PharmShed/Code-review/All-Results/NewNNApproach/expanded/top_classes_recall_ge_0_5.csv --run-name subgroup_recall_ge_0_5

# to evaluate the final model on 2022 data, run this command:
# python evaluate_newNNapproach.py

SEED = 42
ROOT = Path(__file__).resolve().parents[1]
DATA_DIR = ROOT / "All-Input-Data"
RESULTS_DIR = ROOT / "All-Results" / "NewNNApproach"
...
def load_training_frame(feature_set: str) -> pd.DataFrame:
    ...
    if feature_set == "expanded":
        prescription = cleaned_read_csv(DATA_DIR / "prescription.csv")
        prescription = prescription.drop(columns=[TARGET_COL], errors="ignore")
...
def load_validation_frame(feature_set: str) -> pd.DataFrame | None:
    if feature_set != "demographics":
        return None
    return cleaned_read_csv(DATA_DIR / "data_2022.csv")
