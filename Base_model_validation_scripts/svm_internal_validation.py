# This script runs the saved SVM base model on the 2022 validation dataset.
...
def run_meps_2022_svm_pipeline(data_path, output_filename, preprocessor_path, encoder_path, model_path):
    ...
    age_medians = joblib.load('svm_super_age_medians.joblib')
    strength_medians = joblib.load('svm_super_strength_medians.joblib')
    day_supply_medians = joblib.load('svm_super_day_supply_medians.joblib')
    ...
    assert not missing_keys, (
        f"svm_super_age_medians.joblib is missing required key(s): {missing_keys}. "
        "This usually means the file is stale — re-run fit_age_medians(super_df) "
        "in the training script and re-save svm_super_age_medians.joblib before "
...
if __name__ == "__main__":
    ...
    VALIDATION_SUMMARY_FILE = "svm_super_validation_summary.csv"
