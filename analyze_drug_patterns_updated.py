# -*- coding: utf-8 -*-
"""
analyze_drug_patterns.py

Compares demographics and prescribing patterns between two groups of drugs:
  - INCLUDED_DRUGS:     the drug list you're focused on
  - NOT_INCLUDED_DRUGS: the rest of your full drug list (comparison/contrast group)

Uses a prescription-level dataset with columns:
Observation_ID, Person_ID, Household_ID, Drug, Age, Sex, Family_income,
Insurance_coverage, Race_ethnicity, Year, Quantity, Form, Strength, Day_Supply

Outputs:
  - drug_pattern_analysis.xlsx     (multi-sheet: group comparison + per-drug detail)
  - demographic_group_comparison.png
  - prescribing_group_comparison.png
  - demographic_heatmap.png        (per-drug, included list only)
  - prescribing_boxplots.png       (per-drug, included list only)
"""
...
# --------------------------------------------------------------------------
# CONFIG — edit these to point at your real files
# --------------------------------------------------------------------------
DATA_PATH = 'super_data_2022.csv'
FULL_DRUG_LIST_PATH = 'common_years_drug_list.xlsx'  # single column, no header

INCLUDED_DRUGS = ["bimatoprost", "dorzolamide", "fenofibrate", "clavulanate", "clopidogrel", "cyclosporine", "clobetasol", "colchicine", "ipratropium", "ofloxacin", "tamsulosin", "terbinafine", "lactulose", "latanoprost", "timolol", "senna", "linaclotide", "meloxicam", "clonidine", "albuterol", "fluticasone", "tiotropium", "apixaban", "metformin", "azelastine", "alendronate", "triamcinolone", "chlorhexidine", "methylprednisolone", "aspirin", "ketoconazole", "moxifloxacin", "liraglutide", "hydrochlorothiazide", "silver sulfadiazine", "norethindrone", "budesonide", "sumatriptan", "fluorouracil", "brimonidine", "nitroglycerin", "prednisolone", "ciclopirox", "amoxicillin", "mupirocin", "carvedilol", "lidocaine", "azithromycin", "ibuprofen", "ondansetron", "cefdinir", "gabapentin", "methotrexate", "benzonatate", "clindamycin", "olopatadine", "permethrin", "levofloxacin"]
...
# --------------------------------------------------------------------------
# ADVANCED EFFECT SIZE & METRIC MATHEMATICS
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# 1. GROUP-LEVEL COMPARISON: Demographic Analysis (Person-Level)
# --------------------------------------------------------------------------
...
# Process Categorical Demographics (Cramer's V Track)
...
# Process Continuous Demographics (Eta-Squared & Fold Change Track)
...
# --------------------------------------------------------------------------
# 1a2. LOGISTIC REGRESSION: Demographic Controls
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# 1b. ROW-LEVEL COMPARISON: Prescribing Patterns
# --------------------------------------------------------------------------
prescribing_test_results = []
prescribing_continuous = ['Quantity', 'Strength', 'Day_Supply']

# Continuous Prescribing Patterns (Eta-Squared & Fold Change Track)
...
# Categorical Prescribing Form (Cramer's V Track)
...
# --------------------------------------------------------------------------
# 1d. LOGISTIC REGRESSION: No Prescription Controls
# --------------------------------------------------------------------------
